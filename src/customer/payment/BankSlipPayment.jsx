...
const BankSlipPayment = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      formDataToSend.append('bankSlip', bankSlipFile);
      formDataToSend.append('eventId', donationData.eventId);
      formDataToSend.append('amount', donationData.amount);
      formDataToSend.append('email', formData.email);
      formDataToSend.append('firstName', formData.firstName); // ✅ Fixed: Send separate names
      formDataToSend.append('lastName', formData.lastName);   // ✅ Fixed: Send separate names
      formDataToSend.append('mobileNumber', formData.mobileNumber);
      formDataToSend.append('anonymous', donationData.anonymous);
      formDataToSend.append('supportMessage', donationData.supportMessage || '');
      
      if (userId) {
        formDataToSend.append('userId', userId);
      }

      const response = await fetch('http://localhost:3000/payment/bank-slip-donation', {
        method: 'POST',
        body: formDataToSend,
      });
            ...
            <div className="grid md:grid-cols-2 gap-8">
              ...
              <div>
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 space-y-2">
                  ...
                  <div className="flex justify-between">
                    <span className="text-gray-600 font-family-inter">SWIFT Code:</span>
                    <span className="font-medium font-family-inter">{bankDetails.swiftCode}</span>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="bg-gray-50 rounded-lg p-4 space-y-3 mb-6">
                  <div className="flex justify-between">
                    <span className="text-gray-600 font-family-inter">Event:</span>
                    <span className="font-medium font-family-inter">{donationData.eventTitle}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600 font-family-inter">Amount:</span>
                    <span className="font-medium font-family-inter">LKR {donationData.amount.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600 font-family-inter">Donor:</span>
                    <span className="font-medium font-family-inter">
                      {donationData.anonymous ? 'Anonymous' : `${formData.firstName} ${formData.lastName}` || 'Not specified'}
                    </span>
                  </div>
                ...
                <div className="space-y-4">
                  <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                    ...
                    <ul className="text-yellow-700 text-sm space-y-1 font-family-inter">
                      <li>1. Transfer LKR {donationData.amount.toLocaleString()} to the above bank account</li>
                  ...
                  <div className={`border-2 border-dashed rounded-lg p-6 text-center ${
                    errors.bankSlip ? 'border-red-300 bg-red-50' : 'border-gray-300'
                  }`}>
                    <input
                      type="file"
                      id="bankSlip"
                  ...
                  <button
                    ...
                    className={`w-full py-3 rounded-lg font-medium font-family-inter transition-colors ${
                      isSubmitting
