...
const CharityEventsPage = () => {
  ...
  useEffect(() => {
    ...
    updatedEvents = updatedEvents.map(event => ({
      ...event,
      percentage: event.fundingGoal > 0 ? `${Math.round((event.raisedAmount / event.fundingGoal) * 100)}%` : '0%',
    }));
                ...
                <div className="flex space-x-1 bg-gray-100 p-1 rounded-lg w-fit">
                  {['Trending', 'Featured', 'All'].map((tab) => (
                    <button
                      ...
                      className={`px-6 py-2 text-sm font-medium transition-all duration-200 rounded-md font-family-inter ${
                        activeTab === tab
