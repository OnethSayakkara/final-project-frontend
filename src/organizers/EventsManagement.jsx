...
const EventsManagement = () => {
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        const response = await axios.get('http://localhost:3000/event/geteventsbyorganizer/68bb0c220258851dd2262867');
                ...
                <div className="flex justify-between items-start">
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <span className={`px-2 py-1 rounded-md text-xs font-medium ${getStatusColor(event.programmeStatus || event.status)}`}>
                        {event.programmeStatus || event.status}
                      </span>
                    ...
                    {event.type === 'volunteer' && (
                      <div className="flex items-center gap-3 text-sm text-gray-600">
                        ...
                        <span className="font-family-inter">
                          {event.JoinedUsers.length}/{100} registered ({getProgressPercentage(event.JoinedUsers.length, 100)}%)
                        </span>
                        <div className="flex-1 max-w-xs">
                          <div className="w-full bg-gray-200 rounded-full h-2">
                            <div 
                              className="bg-teal-600 h-2 rounded-full transition-all duration-300"
                              style={{ width: `${getProgressPercentage(event.JoinedUsers.length, 100)}%` }}
                            ></div>
                          </div>
                        </div>
                      </div>
                    ...
                    {event.type === 'fundraising' && (
                      <div className="flex items-center gap-3 text-sm text-gray-600">
                        <span className="font-family-inter">
                          {formatCurrency(event.raisedAmount)} of {formatCurrency(event.fundingGoal)} raised ({getProgressPercentage(event.raisedAmount, event.fundingGoal)}%)
                        </span>
                        <div className="flex-1 max-w-xs">
                          <div className="w-full bg-gray-200 rounded-full h-2">
                            <div 
                              className="bg-red-600 h-2 rounded-full transition-all duration-300"
                              style={{ width: `${getProgressPercentage(event.raisedAmount, event.fundingGoal)}%` }}
                            ></div>
                          </div>
                        </div>
                      </div>
